...
if (isset($_POST['nome']) && isset($_POST['matricula']) && isset($_POST['data']) && isset($_POST['patrulhamento'])) {
    ...
    if ($patrulhamento === 'MOTOPATRULHAMENTO') {
        $agendamentos_file = fopen('agendamentos.txt', 'a');
    } elseif ($patrulhamento === 'RADIOPATRULHAMENTO') {
        $agendamentos_file = fopen('agendamentos2.txt', 'a');
    }

    // Ler o arquivo de vagas disponíveis correspondente ao tipo de patrulhamento
    $vagas_disponiveis_file = ($patrulhamento === 'MOTOPATRULHAMENTO') ? 'vagas_disponiveis.txt' : 'vagas_disponiveis2.txt';
...
$agendamento = "Data: $data_agendamento | Nome: $nome | Matrícula: $matricula" . PHP_EOL;
fwrite($agendamentos_file, $agendamento);
...
HTML;
    ...
    if (!$agendamento_realizado) {
        $redirectURL = 'index.php';
$message = "<span style='color: black;'>Alguns dos seguintes erros podem ter ocorrido:</span><br><br>" .
           "1. Você não aguardou 30 segundos antes de fazer um novo agendamento.<br><br>" .
           "2. Alguém agendou essa vaga ao mesmo tempo que você.<br><br>" .
           "3. Não há mais vagas disponíveis para a data escolhida.<br><br>";
